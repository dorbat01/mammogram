...
from src.data.localdata import load_csv, normalize, process_csv, scale_distribution

@pytest.fixture()
def mock_df():
    d = {'BI-RADS': [4,5,4], 'age': [28,74,65], 'shape': [1,1,1], 'margin': [1.0,5.0,np.nan], 'density': [3.0, np.nan, 3.0], 'severity': [0,1,0]}
...
def test_process_csv_with_default_value_success(open_mock, mock_read_csv, mock_fit_transform, mock_df):
    ...
    assert actual.get('metadata') == None
...
def test_process_csv_with_parameter_values_success(open_mock, mock_read_csv, mock_fit_transform, mock_df):
    ...
    expected_parameters = {'datasetname':expected_datasetname, 'target_filename':test_filename, 'metadata':expected_metadata}
    mock_read_csv.return_value = mock_df
    expected_scaled_features = np.array([2,1,2,1])
